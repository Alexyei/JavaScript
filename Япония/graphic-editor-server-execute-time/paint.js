...
// Создание переменной для траектории линий
window.storage = {};
window.storage.data = [];
let timerStart;
let stroke = [[], [], []];
...
// тест скорости рисования с помощью картинки
...
function mousedown(canvas, event) {
    ...
    if (currentSender) {
        ...
        pushStroke(firstPoint.x,firstPoint.y);
...
function mousemove(canvas, event) {
    if (currentSender && startPaint) {
        ...
        pushStroke(secondPoint.x,secondPoint.y);
        redraw();
    }
}
...
function mouseup(canvas, event) {
    if (startPaint) {
        window.storage.data.push(stroke);
